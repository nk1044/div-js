...
function Dashboard03({ previewMode = true }) {
    ...
    // Determine container dimensions based on preview mode
    const containerClasses = previewMode
        ? 'w-full h-[600px] overflow-auto rounded-xl'
        : 'w-full min-h-screen pb-10';

    return (
        <div className={`${containerClasses} bg-gradient-to-b from-neutral-950 to-neutral-800`}>
            <div className='h-full w-full'>
                {/* Header */}
                <nav className="w-full pt-2 sm:pt-4 px-2 text-white">
                    <div className="flex justify-between items-center p-2">
                        {/* Logo */}
                        <div className='flex items-center mr-10'>
                            <img src="/logo.png" alt="logo" className='w-10 sm:w-15' />
                            <div className="bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 bg-clip-text text-transparent text-xl sm:text-3xl font-bold">
                                TrydivJS
                            </div>
                        </div>

                        {/* Desktop Menu */}
                        <ul className="hidden sm:flex gap-2 sm:gap-5 bg-neutral-900 rounded-3xl px-1 py-1 sm:py-2 ml-2 sm:ml-10">
                            {navigationItems.map((item) => (
                                <li
                                    key={item.name}
                                    className="text-white text-sm sm:text-base cursor-pointer transition duration-300 
                                    hover:scale-110 rounded-3xl border border-transparent 
                                    hover:border-neutral-300 hover:bg-neutral-300 hover:text-neutral-900 px-1 sm:px-4 py-1"
                        ...
                        <div className="hidden sm:flex items-center ml-6 gap-2">
                            <button className="border border-neutral-800 cursor-pointer rounded-3xl px-3 sm:px-6 py-1 sm:py-2 bg-neutral-800 
                                transition duration-300 hover:scale-105 text-sm sm:text-base"
                                onClick={() => HandleAuth('Login')}>
                                Login
                            </button>
                            <button className="border border-neutral-800 hidden md:block cursor-pointer rounded-3xl px-3 sm:px-6 py-1 sm:py-2 bg-neutral-200 text-neutral-800 
                                transition duration-300 hover:scale-105 text-sm sm:text-base"
                    ...
                    <div className={`sm:hidden ${menuOpen ? "block" : "hidden"} transition duration-300`}>
                        <ul className="flex flex-col gap-2 bg-neutral-800 rounded-3xl p-2 mx-2">
                            {navigationItems.map((item) => (
                                <li
                                    key={item.name}
                        ...
                        {/* Mobile Buttons */}
                        <div className="flex flex-col gap-2 p-2 mx-2 mt-2">
                ...
                {/* Main Content */}
                <main className="w-full px-4 mt-4 sm:mt-8">
                    <div className="flex items-center justify-center h-full">
                        <div className="relative inline-block">
                            ...
                            <h1 className="relative text-sm sm:text-md text-neutral-400 cursor-pointer rounded-3xl px-4 sm:px-7 py-1 sm:py-2"
                                onClick={() => HandleNavigation('services')}>
                                try the fastest ui library <span className='hover:text-white transition-colors duration-300'>→</span>
                            </h1>
                        </div>
                    </div>
                    <div className="flex flex-col items-center h-full mt-4 sm:mt-6 text-white">
                        <h1 className="text-2xl sm:text-4xl md:text-5xl font-light text-center">Welcome to TrydivJS</h1>
                        <p className="text-sm sm:text-lg text-neutral-400 max-w-2xl text-center pt-2 sm:pt-4">trydivjs - The laziest way to build UI components with just one line. No setup, no hassle, just powerful UI components instantly!</p>
                    </div>
                    <div className="flex justify-center mt-4 sm:mt-6">
                        <button className="group w-32 sm:w-40 h-10 sm:h-12 bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 
                            transform transition duration-300 hover:scale-110 cursor-pointer rounded-3xl">
                            <span className="text-white text-base sm:text-lg font-medium">Get Started!</span>
                        </button>
                    </div>

                    {/* Features - Only show first feature in preview mode */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 place-items-center mt-6 px-4 md:px-6">

                        <div className="group w-full bg-neutral-800/60 backdrop-blur-sm border border-neutral-700/50 p-4 md:p-8 rounded-2xl shadow-xl transition-all duration-300 hover:border-blue-500/30 hover:translate-y-[-5px]">
                            <div className="bg-gradient-to-br from-blue-500 to-blue-600 w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center mb-4 md:mb-6 shadow-lg shadow-blue-500/20">
                            ...
                            <h3 className="font-bold text-lg md:text-xl mb-2 md:mb-3 text-white group-hover:text-blue-400 transition-colors">One-Liner Magic</h3>
                            <p className="text-neutral-400 text-sm md:text-base overflow-hidden">
                                No setup, no headaches—just import and use. Build beautiful UI components with just one line of code. It's that simple!
                            </p>
                            <div className="mt-4 md:mt-6 pt-4 md:pt-6 border-t border-neutral-700/50">
                                <div className="inline-flex items-center cursor-pointer text-blue-400 hover:text-blue-300 font-medium text-sm md:text-base"
                                    onClick={() => HandleNavigation('services')}>
                                    Learn more
                        ...
                        <div className="group w-full bg-neutral-800/60 backdrop-blur-sm border border-neutral-700/50 p-4 md:p-8 rounded-2xl shadow-xl transition-all duration-300 hover:border-purple-500/30 hover:translate-y-[-5px]">
                            <div className="bg-gradient-to-br from-purple-500 to-purple-600 w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center mb-4 md:mb-6 shadow-lg shadow-purple-500/20">
                                <svg className="w-6 h-6 md:w-7 md:h-7 text-white" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <h3 className="font-bold text-lg md:text-xl mb-2 md:mb-3 text-white group-hover:text-purple-400 transition-colors">Insanely Fast</h3>
                            <p className="text-neutral-400 text-sm md:text-base overflow-hidden">
                                Super lightweight, zero dependencies, and optimized for speed. Build and deploy your projects faster than ever before.
                            </p>
                            <div className="mt-4 md:mt-6 pt-4 md:pt-6 border-t border-neutral-700/50">
                                <div className="inline-flex cursor-pointer items-center text-purple-400 hover:text-purple-300 font-medium text-sm md:text-base"
                                    onClick={() => HandleNavigation('services')}>
                                    Learn more
                                    <svg className="w-4 h-4 ml-2 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path>
                        ...
                        <div className="group w-full bg-neutral-800/60 backdrop-blur-sm border border-neutral-700/50 p-4 md:p-8 rounded-2xl shadow-xl transition-all duration-300 hover:border-cyan-500/30 hover:translate-y-[-5px]">
                            <div className="bg-gradient-to-br from-cyan-500 to-cyan-600 w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center mb-4 md:mb-6 shadow-lg shadow-cyan-500/20">
                                <svg className="w-6 h-6 md:w-7 md:h-7 text-white" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <h3 className="font-bold text-lg md:text-xl mb-2 md:mb-3 text-white group-hover:text-cyan-400 transition-colors">Pre Built Components</h3>
                            <p className="text-neutral-400 text-sm md:text-base overflow-hidden">
                                Choose from a wide range of pre-built components to kickstart your project. Customize and style them to fit your needs.
                            </p>
                            <div className="mt-4 md:mt-6 pt-4 md:pt-6 border-t border-neutral-700/50">
                                <div className="inline-flex cursor-pointer items-center text-cyan-400 hover:text-cyan-300 font-medium text-sm md:text-base"
                                    onClick={() => HandleNavigation('services')}>
                                    Learn more
                                    <svg className="w-4 h-4 ml-2 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path>
                ...
                {/* Footer - conditionally shown based on preview mode */}
